...
export default function Register({ setAccessToken, onRegisterSuccess }: RegisterProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (onRegisterSuccess) {
        onRegisterSuccess(response.data.createdUser);
      }
  ...
  return (
    <div className="text-white flex min-h-screen items-center justify-center bg-black font-['Inter',sans-serif] px-4 py-12 sm:px-6 lg:px-8">
      <div className="w-full max-w-md space-y-8 rounded-xl border border-zinc-800/60 bg-gray p-8 shadow-sm">
        <div>
        ...
        {error && (
          <div className="rounded-lg bg-zinc-200 p-4 text-xs text-black_TEMP border border-zinc-800">
        ...
        <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
          ...
          <div className="pt-2">
            <button
              type="submit"
              disabled={loading}
              className="flex w-full justify-center rounded-lg bg-[#151515]_TEMP py-2.5 px-4 text-sm font-medium text-black transition-all duration-150 bg-white hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-black focus:ring-offset-2 disabled:bg-gray-300"
            >
              {loading ? (
